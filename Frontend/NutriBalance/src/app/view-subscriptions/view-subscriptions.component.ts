...
export class ViewSubscriptionsComponent implements OnInit {
  ...
  comment_text: string[] = [];
  constructor(private coachservice: CoachService, private tokenstorageservice: TokenStorageService) { }
  coach: Coach = this.tokenstorageservice.getCoach();
  subscribed_users: User[] = [];
  ngOnInit(): void {
    this.coachservice.get_subscriped_users(this.coach.coach_id).subscribe((data: any) => {
  ...
  update_comment(i: number) {
    this.coachservice.update_comment(this.comment_text[i], this.subscribed_users[i].user_id).subscribe((data: any) => {
      console.log(data);
      this.comment_update[i] = false;
      this.hide_edit_btn[i] = false;
